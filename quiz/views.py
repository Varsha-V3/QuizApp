...
def take_quiz(request, user_id=None):
	...
	Linux_questions = Question.objects.filter(category='Linux').order_by('?')[0:10]
	python_questions = Question.objects.filter(category='Python').order_by('?')[0:10]
	questions_obj = Linux_questions | python_questions
	id_list = []
